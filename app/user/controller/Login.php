...
class Login extends Controller{
    ...
    public function index(){
        if(request()->isPost()) {
            $data = input('post.');
            $table = db('users');
            if($res = $table->where($data)->find()){
                session('user_id',$res['id']);
                $request = Request::instance();
                $arr['last_login'] = time();
                $arr['last_ip'] = $request->ip();
                $table->where($data)->update($arr);
            }else{
                $data['reg_time'] = time();
                $user_id = $table->insertGetId($data);
                session('user_id',$user_id);
            }
            return ['code'=>1,'msg'=>'成功','url'=>'/user/index/index'];

        }

        $this->assign('title','学历中心登录');
